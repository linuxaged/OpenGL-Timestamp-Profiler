...
void fpsCount(GLFWwindow* window, const char* base_title)
{
...
int main()
{
	...
	glfwWindow = glfwCreateWindow(WIN_WIDTH, WIN_HEIGHT, BASE_TITLE, NULL, NULL);
	if(glfwWindow == NULL)
	{
	...
	glfwGetWindowSize(glfwWindow, &win_w, &win_h);
	...
	do
	{
		...
		fpsCount(glfwWindow, BASE_TITLE);
	...
	while (glfwGetKey(glfwWindow, GLFW_KEY_ESCAPE) != GLFW_PRESS && !glfwWindowShouldClose(glfwWindow));
